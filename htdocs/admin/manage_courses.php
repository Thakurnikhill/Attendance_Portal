...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    if ($stmt->execute()) {
        $msg = "<div class='success-message'>Course deleted successfully!</div>";
    } else {
        $msg = "<div class='error-message'>Error deleting course.</div>";
    }
    $stmt->close();
}
...
$courses = [];
$sql = "SELECT c.course_id, c.course_code, c.course_name, c.credits, d.dept_name
        FROM course c
...
<head>
    ...
    <style>
        ...
        @media (max-width: 768px) {
            ...
            .courses-table { font-size: 0.97em; }
            .topbar { padding: 0 10px; }
        }
        @media (max-width: 500px) {
            .courses-table th, .courses-table td { padding: 8px 6px; }
        }
        /* Logout Modal Styles */
        .custom-modal {
            display: none;
        ...
        <?php foreach ($courses as $course): ?>
            <tr>
                ...
                <td class="action-buttons">
                    <a href="edit_course.php?id=<?= $course['course_id'] ?>" class="btn btn-edit">Edit</a>
